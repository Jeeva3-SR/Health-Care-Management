...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Convert backend role to lowercase for comparison
...
console.log("Lowercase Role:", userRole);
      ...
      if (role === "patient" && userRole === "patient") {
        login(backendUser, token, backendUser.role);
        navigate("/patient/dashboard");
        return;
      }
      ...
      setError(
        `Authorized profile found, but it does not match the selected tier: ${role.toUpperCase()}.`,
      );
    } catch (apiError) {
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-indigo-900 via-blue-900 to-indigo-950 text-white flex-col justify-between p-16 relative overflow-hidden">
        ...
        <div className="flex items-center space-x-3 z-10">
          <div className="h-10 w-10 rounded-xl bg-white/10 backdrop-blur-md flex items-center justify-center border border-white/20 shadow-inner">
            <span className="font-black text-xl tracking-wider text-indigo-200">
              H
            </span>
          </div>
        ...
        <div className="max-w-md space-y-4 z-10">
          <h1 className="text-4xl font-extrabold tracking-tight leading-tight">
            A unified workflow platform for medicine.
          </h1>
          <p className="text-slate-300 text-lg leading-relaxed">
            Access your medical history, clinical consultation schedules,
            diagnostic evaluation streams, and healthcare settlements securely
            in one centralized station.
          </p>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 md:p-12 bg-slate-50">
        <div className="w-full max-w-md bg-white border border-slate-100 shadow-xl rounded-2xl p-8 md:p-10 transform transition-all duration-300 hover:shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Dropdown Field - Patient, Doctor & Admin */}
            <div>
          ...
          <div className="mt-8 pt-6 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center text-xs space-y-2 sm:space-y-0">
            {role === "patient" && (
              ...
              >
                New Patient? Register account &rarr;
              </Link>
            )}
            {role === "doctor" && (
              <Link
                ...
                className="text-indigo-600 hover:text-indigo-800 font-semibold transition-colors duration-200"
              >
                Join as a Provider? Register here &rarr;
              </Link>
            )}
            {role === "admin" && (
            ...
            <Link
              to="/about"
              className="lg:hidden text-slate-500 hover:text-slate-800 underline transition-colors duration-200"
            >
              About Us
            </Link>
          </div>
